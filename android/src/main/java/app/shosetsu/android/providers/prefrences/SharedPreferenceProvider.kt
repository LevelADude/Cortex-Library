	...
	private data class PreferenceHolder(
		...
		private inner class SharedPreferenceFlowState :
			...
			override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String?) {
				// Evaluate what setting key [key] corresponds to
				val changedKey = key ?: return
				val resolvedKey: SettingKey<*> =
					SettingKey.valueOf(changedKey) ?: when (changedKey.substringBefore("_")) {
						"int" -> SettingKey.CustomInt(changedKey.substringAfter("_"), 0)
						"string" -> SettingKey.CustomString(changedKey.substringAfter("_"), "")
						"boolean" -> SettingKey.CustomBoolean(changedKey.substringAfter("_"), false)
						"long" -> SettingKey.CustomLong(changedKey.substringAfter("_"), 0L)
						"float" -> SettingKey.CustomFloat(changedKey.substringAfter("_"), 0f)
						"stringSet" -> SettingKey.CustomStringSet(changedKey.substringAfter("_"), setOf())
						else -> {
							logE("No SettingKey has been found matching ($changedKey)")
							return
						}
					}

				when (resolvedKey.default) {

					is String -> {
						@Suppress("UNCHECKED_CAST")
						resolvedKey as StringKey
						val value = getString(resolvedKey)
						stringMap[resolvedKey]?.let { it.value = value } ?: MutableStateFlow(value).also {
							stringMap[resolvedKey] = it
						}
					}

					is Int -> {
						@Suppress("UNCHECKED_CAST")
						resolvedKey as IntKey
						val value = getInt(resolvedKey)
						intMap[resolvedKey]?.let { it.value = value } ?: MutableStateFlow(value).also {
							intMap[resolvedKey] = it
						}
					}

					is Boolean -> {
						@Suppress("UNCHECKED_CAST")
						resolvedKey as BooleanKey
						val value = getBoolean(resolvedKey)
						booleanMap[resolvedKey]?.let { it.value = value } ?: MutableStateFlow(value).also {
							booleanMap[resolvedKey] = it
						}
					}

					is Long -> {
						@Suppress("UNCHECKED_CAST")
						resolvedKey as SettingKey<Long>
						val value = getLong(resolvedKey)
						longMap[resolvedKey]?.let { it.value = value } ?: MutableStateFlow(value).also {
							longMap[resolvedKey] = it
						}
					}

					is Float -> {
						@Suppress("UNCHECKED_CAST")
						resolvedKey as FloatKey
						val value = getFloat(resolvedKey)
						floatMap[resolvedKey]?.let { it.value = value } ?: MutableStateFlow(value).also {
							floatMap[resolvedKey] = it
						}
					}

					is Set<*> -> {
						@Suppress("UNCHECKED_CAST")
						resolvedKey as StringSetKey
						val value = getStringSet(resolvedKey)
						stringSetMap[resolvedKey]?.let { it.value = value }
							?: MutableStateFlow(value).also {
								stringSetMap[resolvedKey] = it
							}
					}
				}
			}
