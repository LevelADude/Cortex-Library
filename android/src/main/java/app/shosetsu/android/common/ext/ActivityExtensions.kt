...
fun AppCompatActivity.setTheme(theme: AppThemes) {
	...
	when (theme) {
		AppThemes.FOLLOW_SYSTEM -> {
			...
			setTheme(if (isSystemDark) R.style.AppTheme_Main else R.style.AppTheme_Main_Light)
		...
		AppThemes.EMERALD_MANUSCRIPT -> {
			setTheme(R.style.AppTheme_Main_Emerald)
			delegate.localNightMode = AppCompatDelegate.MODE_NIGHT_NO
		}
		AppThemes.MIDNIGHT_INK,
		AppThemes.MIDNIGHT_INK_GOLD -> {
			setTheme(R.style.AppTheme_Main_Midnight)
			delegate.localNightMode = AppCompatDelegate.MODE_NIGHT_NO
		}
	}
}
