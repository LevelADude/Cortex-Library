			...
			items(rows, key = { row -> row.extensionID }) { row ->
				...
				SearchRowContent(
					...
					exception = {
						...
						if (exception != null) {
							ExceptionBar(
								remember(exception) { StableHolder(exception!!) },
								onRefresh = { onRefresh(row.extensionID) }
							)
						} else {
							val refreshState = children.loadState.refresh
							if (refreshState is LoadState.Error) {
								ExceptionBar(
									remember(refreshState.error) { StableHolder(refreshState.error) },
									onRefresh = { children.refresh() }
								)
							} else if (shouldShowEmptyState(refreshState, children.itemCount)) {
								EmptyStateMessage(
