...
import app.shosetsu.android.ui.common.shouldShowEmptyState
...
class CatalogFragment : ShosetsuFragment(), ExtendedFABController, MenuProvider {
	...
	override fun onPrepareMenu(menu: Menu) {
		...
		optionSyncJob =
			viewModel.novelCardTypeLive.collectLA(this@CatalogFragment, catch = {}) {
				when (it) {
					NORMAL -> menu.findItem(R.id.view_type_normal)?.isChecked = true
					COMPRESSED -> menu.findItem(R.id.view_type_comp)?.isChecked = true
					COZY -> menu.findItem(R.id.view_type_cozy)?.isChecked = true
					EXPANDED -> menu.findItem(R.id.view_type_expanded)?.isChecked = true
				}
			}
	...
	override fun onMenuItemSelected(item: MenuItem): Boolean =
		when (item.itemId) {
			...
			R.id.view_type_expanded -> {
				item.isChecked = true
				viewModel.setViewType(EXPANDED)
				true
			}
...
fun CatalogContent(
	...
	Box(
		...
		Box(Modifier.pullRefresh(pullRefreshState)) {
			...
			val showEmptyState = shouldShowEmptyState(refreshState, items.itemCount)
			...
			LazyVerticalGrid(
				modifier = Modifier.fillMaxSize(),
				columns = when (cardType) {
					COMPRESSED -> GridCells.Adaptive(400.dp)
					EXPANDED -> GridCells.Fixed(3)
					else -> GridCells.Adaptive(size)
				},
				...
				if (showEmptyState) {
					item(span = { GridItemSpan(maxLineSpan) }) {
						Column(
							...
							Text(
								text = stringResource(R.string.listing_empty_fallback_subtitle),
